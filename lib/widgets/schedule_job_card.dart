...
class ScheduleJobCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double rate = job.job!.shift!.ratePerHr!;
    final String vendor = job.job!.employer!.name!;
    final String time;
    String startTime = DateFormat.Hm().format(job.job!.shift!.startTime!.toLocal());
    String endTime = DateFormat.Hm().format(job.job!.shift!.endTime!.toLocal());
    final String logo = job.job!.employer!.logo!;
    final String location = job.job!.location!.formattedAddress!;
    final bool tips = job.job!.shift!.tip!;
    final bool recurring = job.job!.shift!.days!.length > 1 ? true : false;

    if (dateWithTime && !recurring) {
      String date = DateFormat.yMMMMd().format(job.job!.shift!.days![0].toLocal());
                        ...
                        children: [
                          ...
                          SizedBox(
                            ...
                            child: ListView.builder(
                              shrinkWrap: true,
                              itemCount: job.job!.shift!.days!.length,
                              itemBuilder: (context, index) {
                                return Text(
                                  DateFormat.yMMMMEEEEd().format(job.job!.shift!.days![index]),
                                  style: GoogleFonts.montserrat(
                                    fontSize: 12,
                                    fontWeight: FontWeight.w600,
                                    letterSpacing: 2,
